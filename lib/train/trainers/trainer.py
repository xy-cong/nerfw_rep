...
class Trainer(object):
    ...
    def train(self, epoch, data_loader, optimizer, recorder): 
        ...
        for iteration, batch in enumerate(data_loader):
            # exit()
            # print(batch['ts'].shape, batch['rays'].shape, batch['rgb'].shape) # torch.Size([763, 10000]) torch.Size([763, 10000, 8]) torch.Size([763, 10000, 3])
            # import ipdb; ipdb.set_trace()
            data_time = time.time() - end
            iteration = iteration + 1

            batch = to_cuda(batch, self.device) # 在train的阶段把batch从data_loader里取到送进cuda去！！！！！
