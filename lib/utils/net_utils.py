...
def load_model(net,
    ...
    pretrained_model = torch.load(
        os.path.join(model_dir, '{}.pth'.format(pth)), 'cpu') # why load to cpu?
    # pretrained_model = torch.load(
    #     os.path.join(model_dir, '{}.pth'.format(pth)), map_location={'cuda:0':'cuda:3'})
...
def save_model(net, optim, scheduler, recorder, model_dir, epoch, last=False):
    ...
    pths = [
        int(pth.split('.')[0]) for pth in os.listdir(model_dir) if pth != 'latest.pth'
    ]
    if len(pths) <= 5: # 只保存5个pth+latest
        return
    os.system('rm {}'.format(
        os.path.join(model_dir, '{}.pth'.format(min(pths)))))


def load_network(net, model_dir, resume=True, epoch=-1, strict=True): # test 的时候会load这个
    if not resume:
        return 0
    ...
    pretrained_model = torch.load(model_path, map_location={'cuda:0':'cuda:3'})
